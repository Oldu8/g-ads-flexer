...
from google.ads.googleads.v25.services.services.audience_insights_service import (
    AudienceInsightsServiceClient,
)
from google.ads.googleads.v25.services.types.audience_insights_service import (
...
async def test_generate_insights_finder_report(
    ...
    # User interests are carried as a single attribute group with one
    # AudienceInsightsAttribute per interest (topic_audience_combinations).
    assert len(specific_audience.topic_audience_combinations) == 1
    interest_attributes = specific_audience.topic_audience_combinations[0].attributes
    assert len(interest_attributes) == 2
    assert (
        interest_attributes[0].user_interest.user_interest_category
        == "customers/1234567890/userInterests/12345"
    )

    # Note: GenerateInsightsFinderReportRequest has no dimensions field
